...
TASK-020/021 已将这些合同接入本地可见配置器；TASK-022 又建立了不含付款语义的公开 Quote Basket，在同一浏览器中保存 30 天，并提供本地 `/request-a-quote/` 集合页。TASK-027 进一步增加了仅限本地开发的 server-only `POST /api/rfq/intake/` 证明：它验证 RFQ Submission `2.0.0`，对新 intent 执行一次完整 TASK-025 混合行校验，并使用进程内、非持久的 Stub Repository/Sink。它尚未接入客户可见 `/request-a-quote/` 页面，不会清空 Basket，也不具备生产持久化、安全门或飞书写入；未配置、禁用和 production 模式均最终返回 404。精确配置与验证命令见 [`frontend/README.md`](frontend/README.md#local-only-rfq-intake-runtime)，产品配置边界见 [`docs/frontend/PRODUCT_CONFIGURATION_AND_QUOTE_LINE_CONTRACT.md`](docs/frontend/PRODUCT_CONFIGURATION_AND_QUOTE_LINE_CONTRACT.md)。
