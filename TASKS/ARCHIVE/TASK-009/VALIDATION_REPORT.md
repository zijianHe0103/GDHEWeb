...
1. **生产 deep import 暴露了任意 CMS base 与任意 timeout 注入，绕过唯一 public entry。**
   - Frozen design 将 `index.ts` 定义为“唯一文档化的公共入口”，规定调用方只提供 canonical path，不能覆盖 origin，并冻结默认 timeout 为 5000 ms；测试依赖可以使用显式 base/短 timeout，但不得向正式调用方暴露 origin 注入。见 `TASKS/ARCHIVE/TASK-009/OUTPUTS/DESIGN.md:21-33,54-86`。
