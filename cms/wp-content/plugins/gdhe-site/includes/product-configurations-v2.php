...
const GDHE_PRODUCT_CONFIGURATION_V2_SCHEMA_VERSION = '2.0.0';
const GDHE_PRODUCT_CONFIGURATION_V2_SOURCE_META = '_gdhe_product_configuration_v2_source';

function gdhe_validate_product_configuration_v2_request($request)
{
    if (is_object($request)) {
        $params = call_user_func(array($request, 'get_params'));
        foreach (array_keys(is_array($params) ? $params : array()) as $key) {
            if (!in_array((string) $key, array('locale', 'schema', 'path'), true)) {
    ...
    if ((string) gdhe_request_param($request, 'schema', '')
        !== GDHE_PRODUCT_CONFIGURATION_V2_SCHEMA_VERSION) {
        return gdhe_rest_error(
            'gdhe_invalid_schema',
...
function gdhe_product_configuration_v2_policy($value)
{
    if (!is_array($value)
        || !gdhe_product_configuration_exact_keys($value, array('packaging', 'customLength'))) {
        return null;
    }
    ...
    if (!is_array($base)
        || !gdhe_product_configuration_exact_keys($base, array('required', 'selectionMode', 'options'))
        || $base !== array(
            'required' => true,
    ...
    if (!is_array($custom)
        || !gdhe_product_configuration_exact_keys(
            $custom,
            array('enabled', 'articleNumberResolution', 'minimumExclusive', 'maximum', 'decimalPlaces')
        )
        || $custom !== array(
            'enabled' => true,
    ...
    return array('packaging' => $packaging, 'customLength' => $custom);
}

function gdhe_product_configuration_v2_is_fgd_fixture_truth(array $document): bool
{
...
function gdhe_product_configuration_v2_for_post($post)
{
    if (!is_object($post)
    ...
    $raw_source = get_post_meta($post_id, GDHE_PRODUCT_CONFIGURATION_V2_SOURCE_META, true);
    $source = is_string($raw_source) ? json_decode($raw_source, true) : null;
    if (!is_array($source)
        || !gdhe_product_configuration_exact_keys(
            $source,
            array('version', 'sourceClass', 'websiteEligible', 'product', 'articleNumberOptions', 'configurationPolicy')
        )
        || $source['version'] !== GDHE_PRODUCT_CONFIGURATION_V2_SCHEMA_VERSION
    ...
    foreach ($source['articleNumberOptions'] as $candidate) {
        ...
        if (isset($color_labels[$option['color']['code']])
            && $color_labels[$option['color']['code']] !== $option['color']['label']) {
            return null;
        }
    ...
    $policy = gdhe_product_configuration_v2_policy($source['configurationPolicy']);
    ...
    $document = array(
        'apiVersion' => '1',
        'schemaVersion' => GDHE_PRODUCT_CONFIGURATION_V2_SCHEMA_VERSION,
        'locale' => 'en',
    ...
    if ($source['sourceClass'] === 'test_candidate'
        && !gdhe_product_configuration_v2_is_fgd_fixture_truth($document)) {
        return null;
    }
    return $document;
}

function gdhe_product_configuration_v2_documents(): array
{
    $query = new WP_Query(array(
        ...
        'meta_query' => array(array(
            'key' => GDHE_PRODUCT_CONFIGURATION_V2_SOURCE_META,
            'compare' => 'EXISTS',
        )),
    ));
    $documents = array();
    foreach ((array) gdhe_object_value($query, 'posts', array()) as $post) {
        $document = gdhe_product_configuration_v2_for_post($post);
...
function gdhe_rest_product_configurations_v2($request)
{
    $path = gdhe_validate_product_configuration_v2_request($request);
    if (!is_string($path)) {
        return $path;
    }
    $matches = array_values(array_filter(
        gdhe_product_configuration_v2_documents(),
...
function gdhe_rest_product_configurations_versioned($request)
{
    ...
    if ($schema === GDHE_PRODUCT_CONFIGURATION_V2_SCHEMA_VERSION) {
        return gdhe_rest_product_configurations_v2($request);
    }
    return gdhe_rest_product_configurations($request);
}
