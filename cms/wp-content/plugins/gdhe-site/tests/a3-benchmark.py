...
FIXTURES = {
    "home": "/",
    "productAlpha": "/products/task-007-a3-flow-control-alpha/",
    "productBeta": "/products/task-007-a3-flow-control-beta/",
    "productGamma": "/products/task-007-a3-flow-control-gamma/",
    "market": "/markets/task-007-a3-controlled-water-systems/",
    "reference": "/references/task-007-a3-coastal-research-facility/",
    "supportArticle": "/support/configuration/task-007-a3-configure-flow-control/",
    "download": "/downloads/task-007-a3-flow-control-data-sheet/",
}


def request_once(base_url, public_path):
    query = urllib.parse.urlencode({"locale": "en", "path": public_path, "schema": "3.0.0"})
...
def main():
    if len(sys.argv) != 3:
        print("usage: a3-benchmark.py BASE_URL OUTPUT_JSON", file=sys.stderr)
        return 2
    base_url = sys.argv[1]
    ...
    concurrency = 20
    report = {
        "benchmarkVersion": "TASK-007-A3-BENCHMARK-1",
    ...
    for public_path in FIXTURES.values():
        ...
        if warmup["status"] != 200:
            raise RuntimeError("A3 warmup failed for " + public_path + ": " + json.dumps(warmup))
