...
11. TASK-024 的 v1 闭合机器合同继续作为历史权威保留。TASK-026 已在 `TASKS/ARTIFACTS/TASK-026/` 建立五份闭合 Draft 2020-12 v2 Schema，以及新的 RFC 8785 canonical bytes、versioned-secret HMAC、comparison token、Basket snapshot token 和精确清除固定向量；它们是 Next.js intake 的唯一 v2 合同入口。公开标准配置/配件 Article Number 仍是不可信输入，必须通过一次完整 TASK-025 mixed batch 才能形成权威值；权威 custom 保持 `null / sales_follow_up`，且不伪造 TASK-025 响应中不存在的稳定 Product UUID。TASK-027 已创建仅本地启用且 production 404 的运行时 endpoint；持久幂等状态、客户表单和飞书连接仍未创建。
...
8. **询盘、CRM/协作系统、分析与隐私**
   - 主业务动作是 B2B quotation request，不引入购物结算、在线订单或支付。TASK-024 的 v1 历史边界保持不改；TASK-026 已完成基于 Quote Basket `3.0.0` 与 Article Number 的 additive RFQ Submission `2.0.0` 五 Schema、字段映射和固定向量。TASK-027 已将该合同接入本地、非生产、进程内 Stub intake 证明，但不代表客户表单、生产持久状态或飞书连接已经实现。
   - 已完成顺序为：TASK-025 关闭 Article Number/目录配件身份与最多 50 行 mixed batch 缺口，TASK-026 关闭 additive v2 submission/receipt/error/向量缺口，TASK-027 建立 Next.js local-only intake + 进程内 Stub Repository/Sink。后续仍按独立任务与停止门推进：可见英语联系表单和回执 → 生产持久幂等与安全门 → 真实飞书映射只读核查 → 受控飞书 connector/对账 → HTTPS Staging、安全、隐私和运维验收。
