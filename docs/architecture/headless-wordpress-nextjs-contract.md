...
4. TASK-024 冻结的 `PublicRfqSubmissionDraft 1.0.0`、`AuthoritativeRfqDocument 1.0.0` 和 `PublicRfqReceipt 1.0.0` 继续作为历史机器合同保留，不原地修改。TASK-026 已新增基于 Quote Basket `3.0.0` 的 `PublicRfqSubmissionDraft 2.0.0`、`AuthoritativeRfqDocument 2.0.0`、`PublicRfqReceipt 2.0.0` 与 `PublicRfqError 2.0.0` artifact-only 机器合同：ready 标准配置和目录配件可在不可信公开请求中携带 Article Number，ready 自定义长度保持 `articleNumber:null / sales_follow_up`；公开投影继续排除图片、展示名称/型号、内部 UUID、WordPress/飞书 ID、价格、库存、成本、供应商和 secret。TASK-028 已实现仅本地客户表单、30 分钟 intent、客户安全回执、进程内重放和完整快照 accepted 清除；生产幂等持久状态、生产安全门、飞书/CRM/邮件和部署仍未实现。
