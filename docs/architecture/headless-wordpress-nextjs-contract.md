...
1. GDHE 官网是 B2B 询价站，不是面向消费者的在线商城。访客选择所需型号、规格、配件及其他必要选项后提交 quotation request；当前业务范围不提供购物车结算、在线订单确认或在线支付。
   ...
   - TASK-022 的 Quote Basket `1.0.0` 历史版本只使用完整公开配置作为合并身份，不保存 Article Number、内部 Product/Media UUID 或 WordPress/飞书身份。TASK-025 以独立 Quote Basket `3.0.0` 取代该限制：新标准配置和可直接询价目录配件携带 Article Number，自定义长度继续为 `articleNumber:null / sales_follow_up`。Article Number 是公开、非敏感但不可信的订购身份，可存在于 API、HTML/Flight、客户端状态、浏览器存储、网络请求和开发者工具，但不得主动渲染到客户可见文本、可访问名称、配置摘要、推荐卡、Basket 行、live announcement 或客户回执。
   ...
   - TASK-023 的 Quote Basket `2.0.0` 仍作为冻结历史版本。TASK-025 的 `3.0.0` 沿用同一个存储键、精确 30 天 TTL、256 KiB UTF-8 上限和同源 newer-wins 规则，不创建第二个隐藏 Basket。合法 v1/v2 只在内存中迁移：旧自定义配置无损进入 ready custom；旧标准配置进入 `requires_validation` 并等待一次批量权威校验；旧目录配件进入 `requires_readd`，不按型号、名称、图片、类别或关系顺序猜 Article Number，也不进入批量投影。下一次合法修改才写回 v3。
   - TASK-025 新增 RelatedProductCard `2.0.0`、Quote Basket `3.0.0` 和匿名 no-store `POST /gdhe/v1/quote-line-validations`。Next.js 仅在 server-only consumer 中对 `1..50` 条有资格的配置产品/目录配件执行一次有界混合校验；保持输入顺序、整批原子失败，并且不逐行调用公开 `/resolve`、Product Configuration 或 RelatedProductCard。浏览器仍不直接访问 WordPress 批量端点，最终同源 RFQ intake 尚未实现。
...
4. TASK-024 冻结的 `PublicRfqSubmissionDraft 1.0.0`、`AuthoritativeRfqDocument 1.0.0` 和 `PublicRfqReceipt 1.0.0` 继续作为历史机器合同保留，不原地修改。TASK-026 已新增基于 Quote Basket `3.0.0` 的 `PublicRfqSubmissionDraft 2.0.0`、`AuthoritativeRfqDocument 2.0.0`、`PublicRfqReceipt 2.0.0` 与 `PublicRfqError 2.0.0` artifact-only 机器合同：ready 标准配置和目录配件可在不可信公开请求中携带 Article Number，ready 自定义长度保持 `articleNumber:null / sales_follow_up`；公开投影继续排除图片、展示名称/型号、内部 UUID、WordPress/飞书 ID、价格、库存、成本、供应商和 secret。当前仍未实现客户表单、Route Handler、幂等持久状态或飞书写入。
...
6. Next.js 必须把整个公开 Basket 投影视为不可信输入，在服务端用一个有界批量合同重新解析稳定 Product 身份、角色、有效选项、Article Number 和数量单位。配置产品使用 canonical path；无独立详情页的目录配件直接使用 Article Number，不再要求 opaque public quote key，也不能按型号、名称、catalog path、类别、关系顺序、图片或轨道宽度猜测。TASK-025 已建立 `1..50` 条混合行的 WordPress 权威校验合同与 Next.js server-only consumer；标准配置和目录配件必须唯一命中当前有效 Article Number，明确允许的 custom/manual-sales-follow-up 才能以 Article Number 为空进入人工跟进。整批任何一行未知、重复、撤销、未发布、角色/路径/配置/单位不符时全部失败，且不得逐行调用公开 `/resolve`、Product Configuration 或 RelatedProductCard 形成 N+1。
...
11. TASK-024 的 v1 闭合机器合同继续作为历史权威保留。TASK-026 已在 `TASKS/ARTIFACTS/TASK-026/` 建立五份闭合 Draft 2020-12 v2 Schema，以及新的 RFC 8785 canonical bytes、versioned-secret HMAC、comparison token、Basket snapshot token 和精确清除固定向量；它们是后续 Next.js intake 的唯一 v2 合同入口。公开标准配置/配件 Article Number 仍是不可信输入，必须通过一次完整 TASK-025 mixed batch 才能形成权威值；权威 custom 保持 `null / sales_follow_up`，且不伪造 TASK-025 响应中不存在的稳定 Product UUID。当前未创建运行时 endpoint、持久幂等状态或飞书连接。
...
8. **询盘、CRM/协作系统、分析与隐私**
   - 主业务动作是 B2B quotation request，不引入购物结算、在线订单或支付。TASK-024 的 v1 历史边界保持不改；TASK-026 已完成基于 Quote Basket `3.0.0` 与 Article Number 的 additive RFQ Submission `2.0.0` 五 Schema、字段映射和固定向量，但只属于 artifact-only 合同，不代表 Route Handler、表单提交、持久状态或飞书连接已经实现。
   - 已完成的前置合同顺序为：TASK-025 关闭 Article Number/目录配件身份与最多 50 行 mixed batch 缺口，TASK-026 关闭 additive v2 submission/receipt/error/向量缺口。后续仍按独立任务与停止门推进：Next.js intake + 持久幂等 + 隔离 stub sink → 可见英语联系表单和回执 → 真实飞书映射只读核查 → 受控飞书 connector/对账 → HTTPS Staging、安全、隐私和运维验收。
