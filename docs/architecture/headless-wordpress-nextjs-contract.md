...
4. TASK-024 冻结的 `PublicRfqSubmissionDraft 1.0.0`、`AuthoritativeRfqDocument 1.0.0` 和 `PublicRfqReceipt 1.0.0` 继续作为历史机器合同保留，不原地修改。其公开 Basket 投影“不含 Article Number”的限制已被 TASK-025 的最新业务决定取代；未来最终 RFQ runtime 必须先在独立任务中增加基于 Quote Basket `3.0.0` 的 additive submission 版本与新固定向量，允许 Article Number 进入不可信公开请求，同时继续排除图片、内部 UUID、WordPress/飞书 ID、价格、库存、成本、供应商和 secret。当前仍未实现客户表单、Route Handler、幂等持久状态或飞书写入。
...
11. TASK-024 的闭合机器合同位于 `TASKS/ARTIFACTS/TASK-024/schemas/` 与 `MACHINE_CONTRACT.md`，其 receipt/error、RFC 8785 canonical bytes、versioned secret-key HMAC、幂等/保留和 exact Basket snapshot clear 规则仍是后续实现入口门。由于 TASK-025 允许 Article Number 进入公开请求并引入 Basket `3.0.0`，未来 runtime 必须先建立 additive RFQ submission 版本、对应闭合 Schema 与新固定向量；不得改写 TASK-024 历史字节，也不得直接把 v1 投影用于 v3 Basket。当前未创建运行时 endpoint。
