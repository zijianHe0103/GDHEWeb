...
type Props = Readonly<{configuration:PublicProductConfiguratorViewModel}>;
...
export function ProductConfigurator({configuration}:Props){
  ...
  function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setResult((current)=>applyProductConfiguratorSubmission(configuration,current,{lengthChoice,customLength,colorCode,basePackaging,logoPrinting,protectionArrangement:protectionArrangement||null,quantity}));}
  return <section id="configure-product" className={styles.section} aria-labelledby="configurator-title"><div className={styles.introduction}><p className={styles.eyebrow}>Local configuration test</p><h2 id="configurator-title">Configure Your Track</h2><p>Choose the published track specification and packaging details for one temporary quote item.</p></div><div className={styles.card}><form onSubmit={submit} noValidate>
    ...
    <button type="submit">Add to Quote</button></form><p className={styles.notice}>Temporary quote item only — it has not been sent or saved. Refreshing clears it.</p><div className={styles.result} aria-live="polite">{result.latestDraft&&<LatestPublicQuoteDraftSummary draft={result.latestDraft}/>}</div></div></section>;
}
