...
const temporaryPrefix = ".tmp-product-configuration-server-only-";
...
describe("Product Configuration server-only boundary", () => {
  test.each([
...
async function buildClientImport(
  ...
  try {
    ...
    await cp(join(projectRoot, "src", "lib", "cms"), join(temporaryRoot, "src", "lib", "cms"), { recursive: true });
    await cp(join(projectRoot, "src", "types"), join(temporaryRoot, "src", "types"), { recursive: true });
    ...
    await writeFile(join(temporaryRoot, "app", "layout.tsx"), "export default function Layout({ children }: { children: React.ReactNode }) { return <html><body>{children}</body></html>; }");
    await writeFile(join(temporaryRoot, "app", "page.tsx"), `"use client";\nimport * as consumer from "${modulePath}";\nexport default function Page() { void consumer; return null; }\n`);
...
async function stripMarkers(directory: string): Promise<void> {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      await writeFile(path, source.replaceAll('import "server-only";\n\n', ""));
    }
  }
}
